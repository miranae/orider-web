...
import type { RoutingPosition } from "../../services/courseRouting";
import { PLANNED_ROUTE_COLOR, readPlannedRouteColor } from "../../theme/mapColors";

export default function RouteBuilderMap({ waypoints, route, onAdd, labels }: { waypoints: Waypoint[]; route: RoutingPosition[]; onAdd: (point: Waypoint) => void; labels: { region: string; unavailable: string; waypoint: string } }) {
  const token = getMapboxToken();
  const routeColor = typeof window === "undefined" ? PLANNED_ROUTE_COLOR : readPlannedRouteColor();
  ...
  return <div role="region" aria-label={labels.region} className="h-96 overflow-hidden rounded-[var(--r-lg)]">
    <Map mapboxAccessToken={token} mapStyle={MAP_STYLE} initialViewState={DEFAULT_VIEW} onLoad={(e) => applyKoreaCyclingStyle(e.target)} onClick={(e) => onAdd({ lat: e.lngLat.lat, lng: e.lngLat.lng })} dragRotate={false} attributionControl={false} style={{ width: "100%", height: "100%" }}>
      {route.length > 1 && <Source type="geojson" data={geojson}><Layer type="line" paint={{ "line-color": routeColor, "line-width": 5 }} /></Source>}
