 ...
 *
 * **지연 초기화 (perf, 2026-06):** Analytics(gtag.js ~421kB)는 콜드 첫 로드 임계 경로에서
 * 빠지고 main.tsx 가 idle 시점에 initAnalytics() 로 켠다. init 전 호출된 이벤트/사용자
 * 속성은 아래 큐에 보관했다가 init 시 flush — 이벤트 유실 없음(no-op 아님).
 *
...
/**
 * Analytics 지연 초기화 — main.tsx 가 idle(requestIdleCallback) 시점에 1회 호출.
