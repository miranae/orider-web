...
function TsbHistoryChart({ buckets, selectedIndex, hoverIndex, onHover, onSelect, title, width, controllerRef, selectionLabel }: {
  ...
  return <div className="pmc-history__tsb-wrap">
    ...
    <svg ref={controllerRef} className="pmc-history__tsb-chart" viewBox={`0 0 ${width} ${height}`} role="slider" tabIndex={0} aria-label={title}
      ...
      <ChartAxisLine className="pmc-history__zero-axis" data-tsb-zero-axis="true" x1={TSB_LEFT} x2={right} y1={y(0)} y2={y(0)} />
      <ChartGridLine x1={TSB_LEFT} x2={right} y1={bottom} y2={bottom} />
      <text className="pmc-history__zero-label" x={TSB_LEFT - 8} y={y(0) + 4} textAnchor="end">0</text>
...
export default function PmcHistoryPanel({ points, today, canonical, controlledRange, onControlledRangeChange, rangeChoices = RANGES, ctlColor = PMC_LINE_PALETTE.ctl.color, sourceLabel, variant = "card" }: PmcHistoryPanelProps) {
  ...
  const latestBucket = history.buckets[history.buckets.length - 1];
  ...
  return <ChartFrame as="section" variant={variant} className="pmc-history" aria-labelledby={headingId} header={<ChartHeader title={<h2 id={headingId}>{t("history.title")}</h2>} description={t("history.subtitle")} actions={<span role={sourceState ? "status" : undefined} aria-live={sourceState ? "polite" : undefined} className={`pmc-history__source${sourceTone ? ` pmc-history__source--${sourceTone}` : ""}`} data-source-state={sourceState ?? "default"}>{sourceLabel ?? t(sourceKey)}</span>} />}>
    <div className="pmc-history__chart-head"><div className="pmc-history__chart-heading"><span className="pmc-history__canonical-chart">{mode === "years" ? t("history.mode.years") : t("history.chart.title")}</span><span className="pmc-history__unit">{t(`history.unit.${unit}`)}</span></div><ChartLegend items={mode === "years" ? legendItems : [{ label: t("ctl"), color: ctlColor, seriesId: "ctl", strokeWidth: PMC_LINE_PALETTE.ctl.strokeWidth, strokeLinecap: PMC_LINE_PALETTE.ctl.linecap }, { label: t("atl"), color: PMC_LINE_PALETTE.atl.color, seriesId: "atl", dasharray: PMC_LINE_PALETTE.atl.dasharray, strokeWidth: PMC_LINE_PALETTE.atl.strokeWidth, strokeLinecap: PMC_LINE_PALETTE.atl.linecap }, { label: t("tsb"), color: PMC_LINE_PALETTE.tsb.color, seriesId: "tsb", dasharray: PMC_LINE_PALETTE.tsb.dasharray, strokeWidth: PMC_LINE_PALETTE.tsb.strokeWidth, strokeLinecap: PMC_LINE_PALETTE.tsb.linecap }]} />
      {mode === "trend" && <div role="group" aria-label={t("history.range")} className="pmc-history__buttons pmc-history__ranges">{rangeChoices.map((value) => <Button size="sm" variant={range === value ? "outline" : "ghost"} key={value} aria-pressed={range === value} onClick={() => controlledRange == null ? setLocalRange(value) : onControlledRangeChange?.(value)}>{t(`history.range.${value}`)}</Button>)}</div>}
    ...
    {firstSelected && <div className="pmc-history__value-strip" aria-live="polite"><strong>{valueStripLabel}</strong><div className="pmc-history__values">{mode === "years" ? selected.map(({ label, bucket }) => <span key={label} data-pmc-metric="ctl" style={{ borderInlineStartColor: ctlColor }}>{label} · {t("ctl")} <b>{formatValue(bucket.ctl)}</b></span>) : (["ctl", "atl", "tsb"] as const).map((metric) => <span key={metric} data-pmc-metric={metric} style={{ borderInlineStartColor: metric === "ctl" ? ctlColor : PMC_LINE_PALETTE[metric].color }}>{t(metric)} <b>{formatValue(firstSelected.bucket[metric])}</b></span>)}</div></div>}
    <div className="pmc-history__controls"><div role="group" aria-label={t("history.mode")} className="pmc-history__buttons">{(["trend", "years"] as const).map((value) => <Button size="sm" variant={mode === value ? "outline" : "ghost"} key={value} aria-pressed={mode === value} onClick={() => setMode(value)}>{t(`history.mode.${value}`)}</Button>)}</div>{mode === "years" && <div role="group" aria-label={t("history.years")} className="pmc-history__buttons">{history.availableYears.map((year) => <Button size="sm" variant={years.includes(year) ? "outline" : "ghost"} key={year} aria-pressed={years.includes(year)} onClick={() => setYearChoice(years.includes(year) ? years.filter((value) => value !== year) : [...years, year].sort((a, b) => b - a))}>{year}</Button>)}</div>}</div>
    {selectionOptions.length > 0 && <div className="pmc-history__navigation"><label>{t("history.selection")}<Select value={selectedIndex} onChange={(event) => selectIndex(Number(event.target.value))}>{selectionOptions.map((label, index) => <option key={index} value={index}>{label}</option>)}</Select></label><Button size="sm" variant="ghost" aria-label={t("history.previous")} disabled={selectedIndex === 0} onClick={() => selectIndex(selectedIndex - 1)}>←</Button><Button size="sm" variant="ghost" aria-label={t("history.next")} disabled={selectedIndex === selectionOptions.length - 1} onClick={() => selectIndex(selectedIndex + 1)}>→</Button><Button size="sm" variant="ghost" onClick={() => { setSelectedKey(null); setSelectedMonth(null); }}>{t(unit === "day" ? "history.today" : "history.latest")}</Button></div>}
