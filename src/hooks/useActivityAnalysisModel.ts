...
export function useActivityAnalysisModel(
  ...
  // 활동 문서가 도착해 이 경로의 활동임이 확인되기 전에는 소유권이 "모름" 이다 — 개요를 묻지 않는다.
  const overviewOwnership = !!activity && activity.id === activityId ? isActivityOwner : null;
  const overview = useActivityOverview(activityId, overviewOwnership, JSON.stringify([
