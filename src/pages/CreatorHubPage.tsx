    ...
    integrations: [
      {
        ...
        steps: ko
          ? ["예제 스크립트 examples/recipes/weekly-load-report/weekly-load-report.mjs로 HTML/JSON/TXT 산출물을 만듭니다.", "최근 7일과 직전 7일의 거리, 시간, 상승고도, 부하를 비교합니다.", "HTML에는 KPI 카드와 일별 부하 막대 차트를 넣고, Notion에는 public-safe 집계 요약만 저장합니다.", "지도/경로 썸네일은 streams:read를 켠 private HTML에서만 선택적으로 사용합니다."]
          : ["Run examples/recipes/weekly-load-report/weekly-load-report.mjs to create HTML/JSON/TXT outputs.", "Compare distance, duration, elevation, and load for the last 7 days versus the previous 7 days.", "Put KPI cards and a daily load bar chart in HTML; store only public-safe aggregates in Notion.", "Use map/route thumbnails only in private HTML when streams:read is explicitly enabled."],
