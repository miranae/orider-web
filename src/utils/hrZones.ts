...
export interface ActivityHrZoneResolution { zones: DerivedHrZones; maxHr: number; source: ActivityHrSource }
...
export function deriveHrZones({ maxHr, lthr }: { maxHr: unknown; lthr?: unknown }): DerivedHrZones {
  ...
  return {
    ...
    zones: bounds.map(([minPct, maxPct], index) => ({
      ...
      maxBpmExclusive: maxPct === null ? null : Math.ceil(referenceBpm * maxPct / 100),
      color: COLORS[index]!,
    })),
  };
}
...
export function resolveActivityHrZones(input: {
  ...
  let source = selected?.[1] ?? "default";
  let lthr: unknown;
  if (input.isOwner && input.sport === "run") {
    const usesProfileLthr = validBpm(input.profileLthr);
    lthr = usesProfileLthr ? input.profileLthr : input.activityContextLthr;
    if (validBpm(lthr) && lthr < maxHr) source = usesProfileLthr ? "profile_lthr" : "activity_context_lthr";
  }
  return { zones: deriveHrZones({ maxHr, lthr }), maxHr, source };
}
