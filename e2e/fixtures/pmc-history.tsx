...
const points: PmcHistoryPoint[] = basePoints.map((point, index) => ({ ...point,
  loadStatus: index === 1498 ? "unconfirmed" : "snapshot",
...
const wire: FitnessTimeseriesDoc = {
  ...
  startDate: storedPoints[0].date, endDate: storedPoints[storedPoints.length - 1].date, pointCount: storedPoints.length,
  loadSnapshot: { inputRevision: 2, inputDigest: "fixture-input-2", asOf: computedAt,
    inputReadTime: { seconds: computedAt / 1000, nanoseconds: 0 },
    coverageStartDate: basePoints[0].date, coverageEndDate: today,
    points: basePoints.map(point => ({ date: point.date, dailyLoad: point.date === today ? 70 : point.dailyLoad,
      status: "final", quality: "precomputed" })),
  },
  pmc: { status: lifecycle === "processed" ? "processed" : lifecycle === "failed" ? "failed" : "pending",
    attemptId: "fixture-attempt-2", inputRevision: 2, processedInputRevision: lifecycle === "processed" ? 2 : 1,
    asOf: computedAt, deadlineAt: Date.now() + 60_000, errorCode: lifecycle === "failed" ? "fixture_failure" : null },
};
const displayPoints = lifecycle ? describePmcHistory(storedPoints, [wire]) : points;
...
createRoot(document.getElementById("root")!).render(
  <React.StrictMode><I18nextProvider i18n={i18next}>
    <main style={{ maxWidth: "72rem", margin: "0 auto", padding: "var(--space-4)" }}>
      ...
      <PmcHistoryPanel points={displayPoints} today={today} canonical />
    </main>
