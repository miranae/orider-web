...
test("the reuse decision is bound to the exact head commit", () => {
  assert.match(script, /commits\/\$\{HEAD_OID\}\/check-runs/u,
...
test("only a completed successful CI run allows the skip", () => {
  ...
  assert.match(helper, /all\(\.conclusion == "success"\) then "success"/u,
    "every matching check run must have succeeded");
