...
const config = {
  ...
  trainingDecisionCanonicalEnabled: readBoolEnv("VITE_TRAINING_DECISION_CANONICAL_ENABLED") ?? false,
  trainingExecutionEnabled: readBoolEnv("VITE_TRAINING_EXECUTION_ENABLED") ?? false,
  riderWorkoutDeliveryEnabled: readBoolEnv("VITE_RIDER_WORKOUT_DELIVERY_ENABLED") ?? false,
  ...
  // 정본 소비 전환 플래그. 런타임이 읽는 이름은 여기서 전부 나와야 한다 — 하나라도 빠지면
  // 배포된 화면은 undefined 를 받아 조용히 꺼진 채로 남는다(write-runtime-config.test.mjs 가 대조).
  canonicalConsumersEnabled: readBoolEnv("VITE_CANONICAL_CONSUMERS_ENABLED") ?? false,
  canonicalWeatherEnabled: readBoolEnv("VITE_CANONICAL_WEATHER") ?? false,
  canonicalCourseEnabled: readBoolEnv("VITE_CANONICAL_COURSE") ?? false,
  canonicalMaintenanceEnabled: readBoolEnv("VITE_CANONICAL_MAINTENANCE") ?? false,
  canonicalMilestonesEnabled: readBoolEnv("VITE_CANONICAL_MILESTONES") ?? false,
  canonicalRolloutEnabled: readBoolEnv("VITE_CANONICAL_ROLLOUT_ENABLED") ?? false,
