...
if (!hosting) {
  fail("firebase.json must contain hosting config");
} else {
  const predeploy = Array.isArray(hosting.predeploy) ? hosting.predeploy.join(" && ") : String(hosting.predeploy ?? "");
  requireIncludes(predeploy, "scripts/predeploy-guard.mjs", "hosting.predeploy");
  requireIncludes(predeploy, "scripts/check-env.mjs", "hosting.predeploy");

  const globalHeaderRule = hosting.headers?.find((rule) => rule.source === "**");
  if (!globalHeaderRule) {
    fail("firebase.json hosting.headers must contain a global ** rule");
  } else {
    ...
    if (coop !== "unsafe-none") {
      fail(`Cross-Origin-Opener-Policy must be unsafe-none for Firebase Google popup auth, got ${coop ?? "<missing>"}`);
    }

    const csp = headerMap.get("content-security-policy") ?? "";
    requireIncludes(csp, "script-src", "Content-Security-Policy");
    requireIncludes(csp, "https://apis.google.com", "Content-Security-Policy script-src");
    requireIncludes(csp, "frame-src", "Content-Security-Policy");
    requireIncludes(csp, "https://*.firebaseapp.com", "Content-Security-Policy frame-src");
    requireIncludes(csp, "https://www.google.com", "Content-Security-Policy frame-src");
    requireIncludes(csp, "https://www.recaptcha.net", "Content-Security-Policy frame-src");
  }
}
