...
| `streams:read` | 선택 | 본인만 보는 private 리포트에 미니 경로/고도 썸네일 생성 |
...
private 지도 썸네일을 넣고 싶다면 `streams:read` scope가 있는 key를 만들고 아래 옵션을 켭니다.
...
현재 공개 Personal Data API 문서의 활동 DTO에는 `mapImageUrl` 같은 완성 썸네일 URL이 안정 계약으로 포함되어 있지 않습니다. 외부 개발자는 다음 중 하나를 선택해야 합니다.

- 기본값: 지도 없이 집계 차트만 생성
- private 옵션: `streams:read`로 본인 스트림을 읽어 로컬 HTML에 미니 경로를 그림
